%% -------------------------------------------------------------------
%%
%% rc_skel_js_vm: interaction with JavaScript VMs
%%
...
init([Manager, PoolName]) ->
    ...
    case new_context(StackSize, HeapSize) of
        {ok, Ctx} ->
            ...
            rc_skel_js_manager:add_vm(PoolName),
...
load_user_builtins(Ctx) ->
    case app_helper:get_env(rc_skel, js_source_dir, undefined) of
        undefined ->
            ok;
        Path ->
...
read_config(Param, Default) ->
    case app_helper:get_env(rc_skel, Param, 8) of
        N when is_integer(N) ->
            N;
        _ ->
            Default
